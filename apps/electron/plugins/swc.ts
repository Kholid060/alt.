/**
 * https://github.com/alex8088/electron-vite/blob/02e0dff9f4d8c1ade845e549c57222164dbcb383/src/plugins/swc.ts
 */
import { createRequire } from 'node:module';
import type { SourceMap } from 'rollup';
import type {
  TransformConfig,
  Output as TransformResult,
  JscConfig,
} from '@swc/core';
...
async function transformWithSWC(
  ...
  const map: SourceMap =
    sourcemap && result.map ? JSON.parse(result.map) : { mappings: '' };
...
export function swcPlugin(options: SwcOptions = {}): Plugin {
  ...
  return {
    ...
    async transform(
      code,
      id,
    ): Promise<void | { code: string; map: SourceMap }> {
      if (filter(id)) {
