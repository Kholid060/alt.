import type AMessagePort from '#common/utils/AMessagePort';
import { UiCommand, UiCommandInput } from '@repo/ui';
import { createContext, forwardRef, useEffect, useRef } from 'react';
import type { ExtensionStore } from '../stores/extension.store';
import { createExtensionStore } from '../stores/extension.store';
import { useStore } from 'zustand';
import type { MessagePortCallback } from '#common/interface/message-port-events';
...
export function ExtensionStateProvider({
  ...
  useEffect(() => {
    const onQueryChange: MessagePortCallback<'extension:query-change'> = (
      newQuery,
    ) => {
      updateQuery(newQuery);
    };
    const onParentKeydown: MessagePortCallback<'extension:keydown-event'> = (
      event,
    ) => {
