...
const decryptStorageValue = (value: Buffer) => {
  ...
  return parseJSON(decryptedValue, decryptedValue) as ExtensionAPI.storage.Values ?? null;
}
...
onExtensionIPCEvent('storage.get', async ({ extension }, keys) => {
  ...
  const queryResult = await extensionsDB.query.extensionsStorage.findMany({
    ...
    where: (fields, { inArray, and, eq }) => and(eq(fields.extensionId, extension.id), inArray(fields.key, keys))
  });
...
onExtensionIPCEvent('storage.remove', async ({ extension }, key) => {
  await extensionsDB.delete(extensionsStorage).where(
    and(
      eq(extensionsStorage.extensionId, extension.id),
      Array.isArray(key) ? inArray(extensionsStorage.key, key) : eq(extensionsStorage.key, key)
    )
  );
})
...
onExtensionIPCEvent('storage.clear', async ({ extension }) => {
  await extensionsDB.delete(extensionsStorage).where(eq(extensionsStorage.extensionId, extension.id));
});
