...
import { IPCEvents } from '#common/interface/ipc-events';
import './ipc-extension-messages';
import { ipcMain } from 'electron';

export function onIpcMessage<T extends keyof IPCEvents, P extends Parameters<IPCEvents[T]>>(name: T,
  callback: (...args: [Electron.IpcMainInvokeEvent, ...P]) => Promise<ReturnType<IPCEvents[T]>>) {
  ipcMain.handle(name, async (event, ...args) => callback(event, ...args as P));
}
