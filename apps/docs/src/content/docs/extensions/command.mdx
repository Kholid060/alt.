...
The argument values can be accessed in the environment variable for the script command type. The argument property name has a `__ARGS__` prefix, and the argument name itself will converted into a snake case. For example, when the argument name is `file-name`, it can be accessed using the `__ARGS__FILE_NAME` key.
